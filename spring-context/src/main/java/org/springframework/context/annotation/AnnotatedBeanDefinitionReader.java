...
/**
 * 1、将基于注解的BeanClass转换为AnnotatedGenericBeanDefinition
 * 2、将BeanDefinition注册到DefaultListableBeanFactory里
 */
public class AnnotatedBeanDefinitionReader {
	 ...
	 */
	/**
	 * 读取完BeanDefinition的目的是为了完成注册，所以需要一个注册器BeanDefinitionRegistry
	 * 就是要把读取到的BeanDefinition注册到BeanFactory
	...
	<T> void doRegisterBean(Class<T> beanClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		// 根据@Conditional判断是否应该跳过，具体校验逻辑位于对应Condition实现类中，
		// 例如@Profile的对应校验实现类为ProfileCondition
		if (this.conditionEvaluator.shouldSkip(abd.getMetadata())) {
			return;
		}
		// 添加instanceSupplier回调
		// -BeanDefinition的instanceSupplier属性通常作为创建bean对象的特殊回调
		// -在AbstractAutowireCapableBeanFactory#createBeanInstance()方法中，
		// 会根据BeanDefinition中是否有instanceSupplier信息来执行回调
		abd.setInstanceSupplier(instanceSupplier);
		// 指定作用域，解析 @Scope 标签，处理@Scope 标签的 scopeName 和 proxyMode 属性，封装成 ScopeMetadata
		ScopeMetadata scopeMetadata = this.scopeMetadataResolver.resolveScopeMetadata(abd);
		// 设置 BeanDefinition 的 scopeName
		abd.setScope(scopeMetadata.getScopeName());
		// 生成beanName，解析 bean 的名字，
		...
		// BeanDefinition自定义处理拦截，根据形参的BeanDefinitionCustomizer进行自定义处理，可以修改BeanDefinition的信息
