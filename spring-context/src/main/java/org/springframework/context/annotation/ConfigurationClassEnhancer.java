...
/**
 * 内部使用的类。配置类增强器，用于对@Configuration类（Full模式）使用CGLIB增强，生成一个代理子类字节码Class对象
 */
class ConfigurationClassEnhancer {
	 ...
	 */
	//被增强器增强过的配置类，都会自动的让实现此接口（实际是个BeanFactoryAware）接口
	 ...
	 */
	//CGLIB代理对象拦截器。作用：拦截代理类的setBeanFactory()方法，给对应属性赋值
