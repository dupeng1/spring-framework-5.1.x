		...
		try {
			...
			for (HttpMessageConverter<?> converter : this.messageConverters) {
				...
				if (genericConverter != null ? genericConverter.canRead(targetType, contextClass, contentType) :
					...
					// <4.2.1> 如果请求体不为空
					if (message.hasBody()) {
						// 调用 RequestResponseBodyAdviceChain 的 beforeBodyRead 方法，存在 RequestBodyAdvice 则对方法入参进行修改
						HttpInputMessage msgToUse =
								getAdvice().beforeBodyRead(message, parameter, targetType, converterType);
						// 通过该 HttpMessageConverterChain 从请求体中解析出方法入参对象
						body = (genericConverter != null ? genericConverter.read(targetType, contextClass, msgToUse) :
								((HttpMessageConverter<T>) converter).read(targetClass, msgToUse));
						// 调用 RequestResponseBodyAdviceChain 的 afterBodyRead 方法，存在 RequestBodyAdvice 则对方法入参进行修改
						body = getAdvice().afterBodyRead(body, msgToUse, parameter, targetType, converterType);
					}
					// <4.2.2> 如果请求体为空，则无需解析请求体
					else {
						// 调用 RequestResponseBodyAdvice 的 handleEmptyBody 方法，存在 RequestBodyAdvice 则对方法入参进行修改
